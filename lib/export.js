...
require('org.pinf.genesis.lib/lib/api').forModule(require, module, function (API, exports) {
	...
	exports.export = function (sourcePath, targetPath, form) {
        ...
        API.console.debug("export", sourcePath, targetPath);

        return API.Q.denodeify(function(callback) {

            API.console.debug("check exists sourcePath", sourcePath);
			...
			return API.FS.exists(sourcePath, function (exists) {
				...
				function ensureTargetExists (callback) {
                    ...
                    API.console.debug("ensureTargetExists", targetPath);
				...
				function indexSource (callback) {
                    ...
                    API.console.debug("index source", sourcePath);
                    ...
                    return walker.walk(opts, function(err, paths) {
                        ...
                        API.console.debug("index source done", paths.length);
				...
				return ensureTargetExists(function (err) {
					...
					return indexSource(function (err, paths) {
                        ...
                        API.console.debug("follow paths", sourcePath, targetPath);

						// NEVER: Add any logic to modify what is being copied!
						//        This is not the place for that. Use the io.pinf.pio.postsync
...
//                                API.console.debug("check path", path);
                                if (!path) return true;
                                return (!!paths[path]);
                            }
                        }, function (err) {
                            ...
                            API.console.debug("follow paths done");
    ...
    exports.addOrReplaceFile = function (path, data) {
        if (API.FS.existsSync(path)) {
            API.FS.removeSync(path);
        }
        return exports.addFile(path, data);
    }
